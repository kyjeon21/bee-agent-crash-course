...
export class CustomAgent extends BaseAgent<RunInput, RunOutput, RunOptions> {
  ...
  protected async _run(
    ...
    const response = await this.model.createStructure({
      schema: z.object({
        thought: z
          .string()
          .describe(
            "Describe your thought process before coming with a final answer"
          ),
        final_answer: z
          .string()
          .describe(
            "Here you should provide concise answer to the original question."
          ),
      }),
      messages: [
        new SystemMessage(
          "You are a helpful assistant. Always use JSON format for you responses."
        ),
    ...
    const output = response.object as { thought: string; final_answer: string };
